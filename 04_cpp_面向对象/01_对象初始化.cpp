 ...
 *
 *	可以new带参构造的对象，如new Bean("lee2", 27)
 */
#include <iostream>
#include <stdio.h>
using namespace std;
...
public:
	Bean() : age(20), name("lee") // 初始化 age,name
	{
		printf("Bean 无参构造调用 %p\n", this);
	};
...
public:
	...
	Bean bean; // ok;可以初始化无参构造对象；会调用默认构造创建对象
	// Bean bean("Twang", 18); //error;类内的对象不能显式调用非默认构造函数
	Bean *bean_p = nullptr; // ok;不会调用构造,不会创建对象; 注意要初始化为nullptr
public:
	...
	Person(Bean *p)
	{
		printf("person 参数为指针构造 ，p=%p\n", p);
	}

	string toString()
	{
		return name + " , " + to_string(age) + ", bean[ " + bean.toString() + "]";
	}
};
...
int main()
{
	...
	cout << "bean_p : " << pPerson->bean_p << endl; // 有可能不为0
	delete pPerson;

	printf("\n----------------3.构造参数为指针-----------------\n");
	// Person p(new Student);	//ok
	Person p = new Bean; // 这种写法也行，实际调用的是Person构造函数
