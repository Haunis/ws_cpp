 ...
 *			  std::move返回值和转换为T&&的类型的转换函数的返回值等。
 * 
...
int main()
{
	...
	printf("&i = %#x,typeid(i).name()=%s \n", &i, typeid(i).name());
	printf("&l_i = %#x,typeid(l_i).name=%s \n", &l_i, typeid(l_i).name()); //i和l_i地址和类型都相同
